...
public class ExcelUtils {
    ...
    public static List<List<Object>> readExcel(File file) throws Exception {
        ...
        return commonRead(new FileInputStream(file));
    }
    /**
     * 要求excel版本在2007以上
     *
    ...
    public static List<List<Object>> readExcel(FileInputStream fileInputStream) throws Exception {
        return commonRead(fileInputStream);
    }

    private static List<List<Object>> commonRead(FileInputStream fileInputStream) throws Exception {
                ...
                switch (cell.getCellType()) {
                    ...
                    case XSSFCell.CELL_TYPE_NUMERIC:
                        if ("yyyy\"年\"m\"月\"d\"日\";@".equals(cell.getCellStyle().getDataFormatString())) {
                            value = HSSFDateUtil.getJavaDate(cell.getNumericCellValue()).getTime();
                        } else {
    ...
    /**
     * 导出excel
     * @param excel_export_path 导出的excel路径（需要带.xlsx)
     * @param headList  excel的标题备注名称
     * @param fieldList excel的标题字段（与数据中map中键值对应）
     * @param dataList  excel数据
     * @throws Exception
     */
    public static void createExcel(String excel_export_path,
                                   String[] headList,
                                   String[] fieldList,
        ...
        // ===============================================================
        for (int i = 0; i < headList.length; i++) { // 设置excel头（第一行）的头名称
            // 在索引0的位置创建单元格（左上端）
        ...
        for (int n = 0; n < dataList.size(); n++) { // 添加数据
            // 在索引1的位置创建行（最顶端的行）
        ...
        // 新建一输出文件流
        FileOutputStream fos = new FileOutputStream(excel_export_path);
        // 把相应的Excel 工作簿存盘
        workbook.write(fos);
        fos.flush();
        // 操作结束，关闭文件
